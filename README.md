...
## 現在のステータス: プロトコル解読・Python実装 完了／QRコード印刷に未解決の既知の問題あり
...
- **⚠️ 未解決の既知の問題:** QRコード等、自前生成した画像データを送信すると、紙面に**横4分割**
  されて印字される現象が実機で発生する。原因調査を多数実施したが特定には至っていない。
  詳細は [`docs/protocol_spec.md`](docs/protocol_spec.md) の「既知の未解決問題」セクションを参照
