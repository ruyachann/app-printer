...
- 文字が画像化されているか: 未確認（テキスト印刷のキャプチャ比較で判定予定）
- 画像プロトコルの詳細（幅、bit深度、ビット順など）: 未確認。**解析用端末（Android 7.0）が非力なため「画像」メニューが開けずI01〜I06のキャプチャが取得不能。文字テストを優先し、画像キャプチャは別途対応を検討（例: より高性能な端末の確保）**
