...
現在のステップ: **4分割問題の原因調査（継続中）、および引き渡し資料の更新**

**最優先の次の一手:** 本物のアプリに、384px幅いっぱいを使う「左右で内容が異なる」コンテンツ
（長い文字列や、ノートエディタでの複数要素の意図的な配置など）を印刷させ、そのHCIログを
キャプチャして、4つの96px区画それぞれにどうデータが配置されているかを特定する。
