...
| 11 | QRコード通信を解析（専用コマンド or 画像） | 🔁 再検証中 | ノートエディタ内にQR/バーコード機能を発見。「アプリにQR機能なし」の判断を撤回し、実際にQR印刷して比較する |
| 12 | 画像プロトコルを解析 | 🟡 優先度低下 | 文字印刷が疎な1bitビットマップと判明したため、文字キャプチャ自体が画像プロトコルのサンプルとして使える。I01〜I06専用キャプチャは必須ではなくなった |
