...
with Printer("dd:4c:b9:33:22:10") as printer:  # MACアドレスは実機のものに置き換え
    ...
    printer.print_image("photo.png")  # 既定でディザリング有効（写真の階調も綺麗に印字）
```
...
| 文字印刷 (`print_text`) | ✅ **実機確認済み。** 日本語を含め読める大きさで印字できることを確認 |
| QRコード印刷 (`print_qr`) | ✅ **実機確認済み。** きれいな大きなQRコードが1個だけ印字されることを確認 |
| 画像印刷 (`print_image`) | ✅ **実機確認済み。** グレースケール写真（バラの写真）を、ディザリングありで自然な階調表現で印字できることを確認 |

**プロジェクトの主要な成果（2026-08-26 完了）:** 自前で生成した画像データ（QRコードなど）を
プリンターへ送ると、本来1枚のはずの画像が紙面に横4分割されて印字される問題が長期間未解決
だったが、**根本原因を特定・修正し、文字・QR・画像すべてで実機解決を確認した。** 原因は
「1bit白黒、48バイト/行」という誤ったプロトコル理解で、正しくは**「4階調グレースケール、
1バイトに2画素、192バイト/行」**だった。これは本物のアプリ（`com.dingdang.newprint`）のAPKを
取得・逆コンパイルすることで判明した（ブラックボックスの通信解析だけでは特定できなかった）。
写真印刷では4階調ゆえのバンディング（縞模様）が発生したため、Floyd-Steinbergディザリングを
既定で有効にして解決した。詳細な経緯は
...
- ヘッダーの`width_bytes_ref`フィールド（常に48）が、幅384px以外の画像でどう扱われるかは未検証
  （本プロジェクトでは常に384px幅に固定しているため実用上は問題ないが、仕様の完全な理解ではない）

その他の詳細は [`docs/progress.md`](progress.md) の「未確定事項」セクションを参照してください。

## 次の一手（おすすめ）

主要機能（文字・QR・画像印刷）は全て実機確認済みのため、本プロジェクトの主目的は達成された。
余裕があれば、以下がより完成度を高める:

1. `docs/protocol_spec.md`の未確定事項（フッターの意味等）を埋めると、より頑健な実装にできる
2. カラー写真での印字確認（現状はグレースケール写真のみ確認済み。`to_gray4_bitmap()`は
   `image.convert("L")`でグレースケール化してから処理するため、カラー写真もそのまま渡せるはず）
